...
Author: C. Mahood
Date: 2021-10-15
Purpose: To convert the BARC rasters delivered by Hatfield into a single fc, with text BARC values and fire numbers 
...
delivery_dir = r"C:\Data\Burn_Severity\same_year_2023\interim\filtered"
out_gdb_dir = r'C:\Data\Burn_Severity\same_year_2023\interim\products'

scenes_csv = r"C:\Data\Burn_Severity\same_year_2023\interim\output\S2\sceneIds.csv"

gdb_name = 'provincial_burn_severity_interim'
fire_year = '2023'
...
for barc_tif in barc_list:
    ...
    pre_img_date = datetime.strptime(pre_img_date_str, '%Y-%m-%d').date()
    ...
    post_img_date = datetime.strptime(post_img_date_str, '%Y-%m-%d').date()
    arcpy.AddField_management(out_fc, f, "DATE")
...
arcpy.AddField_management(gdb_name, 'PRE_FIRE_IMAGE_DATE', 'TEXT')
arcpy.AddField_management(gdb_name, 'POST_FIRE_IMAGE', 'TEXT')
arcpy.AddField_management(gdb_name, 'POST_FIRE_IMAGE_DATE', 'TEXT')
